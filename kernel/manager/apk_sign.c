...
/*
 * Trusted Manager APKs: v2 scheme only (see check_v2_signature). Match order:
 *   1) KSU_EXPECTED_SIZE + KSU_EXPECTED_HASH (release / primary key, default in Kbuild)
 *   2) If set at build time: EXPECTED_SIZE2 + EXPECTED_HASH2 (e.g. PR CI or
 *      ksu-manager-signing.local.mk for a second signing certificate). Not package-name-only.
 */
