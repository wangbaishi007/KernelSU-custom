...
/** Shell exit 127 = command not found; missing jniLibs is a common cause for custom builds. */
private fun missingBootPatchNativeLibs(): String? {
    val ksud = File(getKsuDaemonPath())
    val magiskboot = File(ksuApp.applicationInfo.nativeLibraryDir, "libmagiskboot.so")
    val missing = buildList {
        if (!ksud.isFile) add("libksud.so")
        if (!magiskboot.isFile) add("libmagiskboot.so")
    }
    if (missing.isEmpty()) return null
    return "Missing ${missing.joinToString()} under ${ksud.parent}. " +
        "Copy ksud (as libksud.so) and libmagiskboot.so into app/src/main/jniLibs/<abi>/ before assemble; " +
        "see manager build docs / AGENTS.md."
}
...
/**
 * Install a module from a local zip path (same as content URI install in [flashModule]).
 */
fun installModuleZipFromFile(
    zipFile: File,
    onStdout: (String) -> Unit,
    onStderr: (String) -> Unit,
): FlashResult {
    val cmd = "module install ${zipFile.absolutePath}"
    val result = flashWithIO("${getKsuDaemonPath()} $cmd", onStdout, onStderr)
    Log.i("KernelSU", "install module file ${zipFile.name} -> ${result.code}")
    return FlashResult(result)
}
...
fun installBoot(
    ...
    missingBootPatchNativeLibs()?.let { msg ->
        Log.e(TAG, msg)
        onStderr(msg)
        return FlashResult(127, msg, false)
    }
